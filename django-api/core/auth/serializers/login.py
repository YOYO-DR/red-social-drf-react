...
class LoginSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        data = super().validate(
            attrs
        )  # llamo la funcion de la clase base, y obtengo los token de refresco y de acceso
        refresh = self.get_token(self.user)  # obtengo el token de refresco también
        data["user"] = UserSerializer(
            self.user
        ).data  # obtengo los datos del usuario creando una instancia del serializer de usuario
